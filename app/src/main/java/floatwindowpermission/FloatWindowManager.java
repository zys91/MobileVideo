...
import floatwindowpermission.rom.HuaweiUtils;
import floatwindowpermission.rom.MeizuUtils;
import floatwindowpermission.rom.MiuiUtils;
import floatwindowpermission.rom.OppoUtils;
import floatwindowpermission.rom.QikuUtils;
import floatwindowpermission.rom.RomUtils;
...
public class FloatWindowManager {
    ...
    public static void RequestPermissions(final Activity mActivity, boolean captureScreenFlag) {
        try {
            // 请求普通权限
            List<String> unHasPerimissions = new ArrayList<>();
            for (String permission : permissionManifest) {
                ...
                if (checkRes != successFlag) {
                    unHasPerimissions.add(permission);
                }
            }
            if (unHasPerimissions.size() > 0) {
                String[] unPers = new String[unHasPerimissions.size()];
                for (int i = 0; i < unPers.length; i++) {
                    unPers[i] = unHasPerimissions.get(i);
                }
                ActivityCompat.requestPermissions(mActivity, unPers, 1);
            }
            if (captureScreenFlag) {
                //请求覆盖窗口权限
                boolean aOverlayFlag = FloatWindowManager.getInstance().applyFloatWindow(mActivity);
                //请求获取屏幕权限
                if (aOverlayFlag) {
            ...
            Log.e(TAG, "acOpenFlowWin: " + er.getMessage());
        }
    }
